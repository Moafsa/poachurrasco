...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-2">Gerencie o catálogo compartilhado entre seus estabelecimentos.</p>
        </div>
        ...
        <form method="GET" class="p-6 grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="flex items-end space-x-2">
                <label class="inline-flex items-center">
                    ...
                    <span class="ml-2 text-sm text-gray-700">Apenas em destaque</span>
                </label>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Atualizado</th>
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-sm text-gray-500">{{ $product->establishment->name ?? 'Biblioteca compartilhada' }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex items-center space-x-2">
                                <a href="{{ route('products.show', $product) }}" class="text-sm text-churrasco-600 hover:text-churrasco-700 font-semibold">Ver</a>
                                <a href="{{ route('products.edit', $product) }}" class="text-sm text-gray-600 hover:text-gray-800 font-semibold">Editar</a>
                                <form action="{{ route('products.destroy', $product) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja remover este produto?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-sm text-red-600 hover:text-red-700 font-semibold">Excluir</button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            Nenhum produto encontrado. Comece criando seu primeiro produto.
                        </td>
                    </tr>
                @endforelse
            </tbody>
